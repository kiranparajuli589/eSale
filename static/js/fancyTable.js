	...
	$.fn.fancyTable = function(options) {
		...
		this.tableSort = function (elm) {
			if(typeof elm.fancyTable.sortColumn !== "undefined" && elm.fancyTable.sortColumn < elm.fancyTable.nColumns){
				...
				let sortArrow = $("<div>",{"class":"sortArrow"}).css({"margin":"0.1em","display":"inline-block","width":0,"height":0,"border-left":"0.4em solid transparent","border-right":"0.4em solid transparent"});
				sortArrow.css(
				...
				let rows = $(elm).find("tbody tr").toArray().sort(
					function(a, b) {
						let stra = $(a).find("td").eq(elm.fancyTable.sortColumn).html();
						let strb = $(b).find("td").eq(elm.fancyTable.sortColumn).html();
		...
		this.each(function() {
			...
			if($(elm).find("thead").length==0){
				...
				//let c=$(elm).find("tr").first().find("td").length;
				//for(let n=0; n<c; n++){
			...
			if(settings.searchable){
				...
				if(settings.globalSearch){
					...
					}).bind("change paste keypress",function(){
						elm.fancyTable.search = $(this).val();
						console.log($(this).val());
						instance.tableUpdate(elm);
					});
					let th = $("<th>",{ style:"padding:2px;" }).attr("colspan",elm.fancyTable.nColumns);
